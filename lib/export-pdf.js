...
// ============================================================================
export async function exportConsolidatedPDF({ year, budget, budgetLines, buList, buStructure, historical, consolidated, inMillions }) {
  const M = inMillions;
  const yr = year;
  const prevYr = yr === "2026" ? null : String(Number(yr) - 1);
  const refLabel = yr === "2026" ? "2025 (Est.)" : String(Number(yr) - 1);

  const { doc, pageW, addFooter } = await createDoc(`Budget Consolide ${yr}`, yr, `Budget Interne ${yr} — Montants en FCFA`);
  ...
  const columns = [
    { label: 'Ligne Budgetaire', width: 80, _isHeader: true },
    { label: `Budget ${yr}` },
    { label: `Ref. ${refLabel}` },
    { label: '2024 (Reel)' },
    { label: '2023 (Reel)' },
  ...
  const makeGetData = (lines, type) => (line) => {
    ...
    return [line, fmt(val, M), fmt(prev, M), fmt(h24, M), fmt(h23, M), pct(variation)];
  };

  const makeTotal = (lines, type) => {
    const totalVal = lines.reduce((s, l) => s + budget.getConsolidatedValue(l, yr, buList, buStructure), 0);
    ...
    const totalVar = totalPrev ? (totalVal - totalPrev) / Math.abs(totalPrev) : null;
    return [
      { label: 'Ligne Budgetaire', _isHeader: true, width: 80 },
      { label: `Budget ${yr}`, total: fmt(totalVal, M) },
  ...
  addFooter();
  triggerDownload(doc, `budget_ipd_consolide_${yr}.pdf`);
}

// ============================================================================
// BU VIEW PDF
// ============================================================================
export async function exportBUViewPDF({ year, budget, budgetLines, buList, buStructure, selectedBU, inMillions }) {
  const M = inMillions;
  const yr = year;
  const bu = selectedBU;
  ...
  const { doc, pageW, addFooter } = await createDoc(bu, yr, `Business Unit — Budget ${yr} — ${units.length} unites`);
  ...
  const makeGetData = (lines) => (line) => {
    ...
    return [line, fmt(val, M), pctVal];
  };

  const makeTotal = (lines) => {
    const totalVal = lines.reduce((s, l) => s + budget.getBUValue(bu, l, yr, buStructure), 0);
    const totalConsol = lines.reduce((s, l) => s + budget.getConsolidatedValue(l, yr, buList, buStructure), 0);
    const totalPct = totalConsol ? (totalVal / totalConsol * 100).toFixed(1) + '%' : '-';
    return [
      { label: 'Ligne Budgetaire', _isHeader: true, width: 100 },
      { label: `Budget ${yr}`, total: fmt(totalVal, M) },
  ...
  addFooter();
  triggerDownload(doc, `budget_ipd_bu_${sanitizeName(bu)}_${yr}.pdf`);
}

// ============================================================================
// UNIT VIEW PDF
// ============================================================================
export async function exportUnitViewPDF({ year, budget, budgetLines, selectedUnit, selectedBU, unitBudgets, inMillions }) {
  const M = inMillions;
  const yr = year;
  ...
  const { doc, pageW, addFooter } = await createDoc(unit, yr, `${bu} — Budget ${yr}`);
  ...
  const bodyRows = ALL_LINES.map(line => {
    ...
    if (Math.abs(val) < 1 && Math.abs(allocated) < 1) return null;
    return [line, fmt(val, M), fmt(allocated, M), isOvr ? 'Modifie' : allocated > 0 ? 'Alloue' : '-'];
  }).filter(Boolean);

  doc.autoTable({
    startY: currentY,
    head: [['Ligne Budgetaire', `Budget ${yr}`, 'Base (calcul)', 'Statut']],
    body: bodyRows,
    theme: 'grid',
    styles: { fontSize: 7, cellPadding: 2, lineColor: COLORS.gray200, lineWidth: 0.1 },
    headStyles: { fillColor: COLORS.headerBg, textColor: COLORS.white, fontSize: 7, fontStyle: 'bold' },
    ...
    columnStyles: {
      ...
      2: { halign: 'right', textColor: COLORS.gray400 },
      3: { halign: 'center' },
    },
    didParseCell: (data) => {
  ...
  doc.text(fmt(total, M) + ' FCFA', 50, finalY);

  addFooter();
  triggerDownload(doc, `budget_ipd_unite_${sanitizeName(unit)}_${yr}.pdf`);
}

// ============================================================================
// DASHBOARD VIEW PDF
// ============================================================================
export async function exportDashboardPDF({ year, budget, budgetLines, buList, buStructure, allUnits, historical, consolidated, inMillions }) {
  const M = inMillions;
  const yr = year;
  const ALL_LINES = [...budgetLines.revenues, ...budgetLines.opex, ...budgetLines.capex];
