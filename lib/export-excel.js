...
// CONSOLIDATED EXCEL
// ============================================================================
export async function exportConsolidatedExcel({ year, budget, budgetLines, buList, buStructure, historical, consolidated, inMillions }) {
  const M = inMillions;
  const yr = year;
  ...
  addTitleRow(ws, `Budget Consolide ${yr}`, colCount);
  ...
  const summaryRow = ws.addRow(['Revenus: ' + fmt(rev, M), '', 'OPEX: ' + fmt(opex, M), '', 'CAPEX: ' + fmt(capex, M), 'Resultat: ' + fmt(net, M)]);
  ...
  addHeaderRow(ws, ['Ligne Budgetaire', `Budget ${yr}`, `Ref. ${refLabel}`, '2024 (Reel)', '2023 (Reel)', 'Var %']);
  ...
  for (const section of sections) {
    ...
    let sectionTotal = 0, sectionPrev = 0, sectionH24 = 0, sectionH23 = 0;
    ...
    section.lines.forEach((line, i) => {
      ...
      sectionTotal += val; sectionPrev += prev; sectionH24 += h24; sectionH23 += h23;

      const row = addDataRow(ws, [line, rawNum(val), rawNum(prev), rawNum(h24), rawNum(h23), variation !== null ? pct(variation) : '-'], i % 2 === 0);
    ...
    const totalVariation = sectionPrev ? (sectionTotal - sectionPrev) / Math.abs(sectionPrev) : null;
    addTotalRow(ws, [`TOTAL ${section.name.toUpperCase()}`, rawNum(sectionTotal), rawNum(sectionPrev), rawNum(sectionH24), rawNum(sectionH23), totalVariation !== null ? pct(totalVariation) : '-'], section.type);
    addEmptyRow(ws);
  }
...
// BU VIEW EXCEL
// ============================================================================
export async function exportBUViewExcel({ year, budget, budgetLines, buList, buStructure, selectedBU, inMillions }) {
  const M = inMillions;
  const yr = year;
  const bu = selectedBU;
  ...
  addSubtitleRow(ws, `${units.length} unites — Budget ${yr}`, colCount);
  addEmptyRow(ws);
  addHeaderRow(ws, ['Ligne Budgetaire', `Budget ${yr}`, '% du Consolide']);
  ...
  for (const section of sections) {
    const filteredLines = section.lines.filter(l => {
      const val = budget.getBUValue(bu, l, yr, buStructure);
      const consolVal = budget.getConsolidatedValue(l, yr, buList, buStructure);
    ...
    filteredLines.forEach((line, i) => {
      ...
      sectionTotal += val;
      addDataRow(ws, [line, rawNum(val), pctVal], i % 2 === 0);
    });

    const totalConsol = section.lines.reduce((s, l) => s + budget.getConsolidatedValue(l, yr, buList, buStructure), 0);
    const totalPct = totalConsol ? (sectionTotal / totalConsol * 100).toFixed(1) + '%' : '-';
    addTotalRow(ws, [`TOTAL ${section.name.toUpperCase()}`, rawNum(sectionTotal), totalPct], section.type);
    addEmptyRow(ws);
  }
  ...
  addHeaderRow(ws, ['Unite', 'Budget Total', '']);
  ...
  unitData.forEach((d, i) => {
    if (d.total > 0) addDataRow(ws, [d.name, rawNum(d.total), ''], i % 2 === 0);
  });
  ...
  await downloadWorkbook(wb, `budget_ipd_bu_${sanitizeName(bu)}_${yr}.xlsx`);
}

// ============================================================================
// UNIT VIEW EXCEL
// ============================================================================
export async function exportUnitViewExcel({ year, budget, budgetLines, selectedUnit, selectedBU, unitBudgets, inMillions }) {
  const M = inMillions;
  const yr = year;
  ...
  addSubtitleRow(ws, `${bu} — Budget ${yr}`, colCount);
  addEmptyRow(ws);
  addHeaderRow(ws, ['Ligne Budgetaire', `Budget ${yr}`, 'Base (calcul)', 'Statut']);
  ...
  ALL_LINES.forEach(line => {
    ...
    if (Math.abs(val) < 1 && Math.abs(allocated) < 1) return;

    const status = isOvr ? 'Modifie' : allocated > 0 ? 'Alloue' : '-';
    const row = addDataRow(ws, [line, rawNum(val), rawNum(allocated), status], rowIdx % 2 === 0);
    ...
    // Highlight overridden budget values in blue
    if (isOvr) {
      row.getCell(2).font = { ...row.getCell(2).font, color: { argb: COLORS.blueText }, bold: true };
    }
  ...
  // Total row
  addEmptyRow(ws);
  const total = ALL_LINES.reduce((s, l) => s + budget.getUnitValue(unit, l, yr), 0);
  const totalRow = ws.addRow(['BUDGET TOTAL', rawNum(total), '', '']);
  totalRow.height = 26;
  totalRow.getCell(1).font = { name: 'Calibri', size: 11, bold: true, color: { argb: COLORS.gray700 } };
  totalRow.getCell(2).font = { name: 'Calibri', size: 12, bold: true, color: { argb: total >= 0 ? COLORS.greenAccent : COLORS.redAccent } };
  ...
  await downloadWorkbook(wb, `budget_ipd_unite_${sanitizeName(unit)}_${yr}.xlsx`);
}

// ============================================================================
// DASHBOARD EXCEL
// ============================================================================
export async function exportDashboardExcel({ year, budget, budgetLines, buList, buStructure, allUnits, historical, consolidated, inMillions }) {
  const M = inMillions;
  const yr = year;
  const ALL_LINES = [...budgetLines.revenues, ...budgetLines.opex, ...budgetLines.capex];
